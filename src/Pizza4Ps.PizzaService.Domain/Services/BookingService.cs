...
namespace Pizza4Ps.PizzaService.Domain.Services
{
    public class BookingService : DomainService, IBookingService
    {
        ...
        public async Task<Guid> CreateAsync(DateTime bookingDate, int guestCount, string status, Guid customerId, Guid tableBookingId)
        {
            var entity = new Booking(Guid.NewGuid(), bookingDate, guestCount, status, customerId, tableBookingId);
        ...
        public async Task<Guid> UpdateAsync(Guid id, DateTime bookingDate, int guestCount, string status, Guid customerId, Guid tableBookingId)
        {
            var entity = await _bookingRepository.GetSingleByIdAsync(id);
            entity.UpdateBooking(bookingDate, guestCount, status, customerId, tableBookingId);
            await _unitOfWork.SaveChangeAsync();
            return entity.Id;
        }
